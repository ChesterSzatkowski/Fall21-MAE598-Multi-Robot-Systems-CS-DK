...
species = 7;
tspan = [0 200]; %seconds
x0 = [0.1, 0, 0.65, 0, 0, 0, 0]; %initials

M = readmatrix('M.txt');

alpha1 = 2; % rate for robots working to move a large object into transportation phase
alpha2 = 1; % rate for robot working to move a small object into transportation phase
alpha3 = 0.5; % rate for large object to be delivered and 4 robots to be freed
alpha4 = 0.5; % rate for small object to be delivered and a robot to be freed
alpha5 = 0.8; % rate for a robot to take 10 small objects to make a large object
beta = 0.0; % rate for robot to disassemble a large object into 10 small objects
...
y = [4*x(1)*x(3); x(4); x(1)*x(2); x(5); x(4); 4*x(1)*x(6); x(5); x(7)*x(1); 10*x(2)*x(1); x(3)];
